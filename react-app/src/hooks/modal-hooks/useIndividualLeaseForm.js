...
export default function useIndividualLeaseForm(
  ...
  const debouncedIdentificationNumber = useDebounce(
  ...
  const getUser = useCallback(() => {
    if (
      ...
      debouncedIdentificationNumber.length >= 5
    ) {
      try {
        setIsLoading(true);
        axios
          ...
          .then((response) => {
            if (response.data?.props.name) {
              ...
              if (
                data.identificationNumber.length === 6 ||
                data.identificationNumber.length > 7
              ) {
