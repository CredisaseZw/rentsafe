...
export const transformCreditData = (creditGiven, creditTaken) => {
  ...
  const totalCreditGiven = transformedCreditGiven.reduce((acc, item) => acc + item.amount, 0) || 0;
  const totalCreditTaken = transformedCreditTaken.reduce((acc, item) => acc + item.amount, 0) || 0;
