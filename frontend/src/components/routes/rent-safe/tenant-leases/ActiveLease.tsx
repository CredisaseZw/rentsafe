...
function ActiveLease() {
    ...
    const [leaseMode, setLeaseMode] = useState("Individual")
    ...
    return (
        ...
        <DialogContent onInteractOutside={(e) => e.preventDefault()} className={`max-w-[1010px] sm:max-w-[default] p-7 max-h-[90vh] overflow-y-auto overflow-x-auto`}>
            <DialogTitle>Add Lease - {capitalizeFirstLetter(leaseMode)}</DialogTitle>
            <div className="mt-3">
                <Tabs defaultValue={tabs[0].value} className="w-full">
                    <TabsList className="flex gap-3">
                        <div className="flex gap-3">
                            <span className="text-sm text-gray-500 self-center">Active:</span>
                            <Select name="leaseStatus" defaultValue={leaseModes.current[0].value} onValueChange={(val)=> setLeaseMode(val)}>
