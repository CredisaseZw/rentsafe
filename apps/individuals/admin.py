...
from apps.individuals.models.models import (
    Individual, EmploymentDetail, NextOfKin, IndividualContactDetail
)
...
class NextOfKinInline(admin.TabularInline):
    model = NextOfKin
    extra = 0
    fields = ('first_name', 'last_name', 'mobile_phone', 'email', 'relationship', 'physical_address')
    show_change_link = True
...
class IndividualAdmin(admin.ModelAdmin):
    list_display = (
        'id','last_name', 'first_name', 'identification_number',
        'identification_type','get_mobile_phone', 
        'created_by', 'gender', 'date_of_birth', 'is_verified',
        'is_active','is_deleted',"date_created"
    )
    list_display_links = ('last_name', 'first_name','identification_number')
    list_filter = (
        'gender', 'identification_type', 'is_active',
        'is_verified','is_deleted',"date_created"
    )
    ...
    def get_mobile_phone(self, obj): 
        if contact := obj.contact_details.filter(type='mobile').first(): 
            ...
            return obj.contact_details.first().phone_number if obj.contact_details.exists() else None 
