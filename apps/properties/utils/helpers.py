...
def create_property_with_unit(property_data, unit_data=None, user=None):
    ...
    try:
        with transaction.atomic():
            ...
            # Remove None values
            property_serializer_data = {
                k: v for k, v in property_serializer_data.items() if v is not None
            }

            # Create property using the serializer
            property_serializer = PropertyDetailSerializer(
                data=property_serializer_data
            )
            ...
            if unit_data:
                ...
                # Remove None values
                unit_serializer_data = {
                    k: v for k, v in unit_serializer_data.items() if v is not None
                }
    ...
    try:
        with transaction.atomic():
            ...
            if property_type_name:
                ...
                try:
                    ...
                    raise ValidationError(
                        f"PropertyType with ID {property_type_id} not found"
                    )
            else:
                raise ValidationError(
                    "Either property_type_name or property_type_id is required"
                )

            print(
                f"DEBUG: Resolved property type: {property_type.id} - {property_type.name}"
            )
            ...
            property_qs = Property.objects.filter(
                ...
                & Q(
                    addresses__street_address__iexact=address_data.get(
                        "street_address", ""
                    )
                )
            ).distinct()
            ...
            # Create property using the serializer
            property_serializer = PropertyCreateSerializer(
                data=property_serializer_data
            )
...
def create_unit_for_property(property_instance, unit_data, user=None):
    ...
    try:
        with transaction.atomic():
            if not unit_data:
                print("DEBUG: No unit data provided, creating default unit")
                if unit := Unit.objects.filter(
                    property=property_instance,
                    unit_number=property_instance.name or "Full Property",
                ).first():
                    print("DEBUG: Default unit already exists, skipping creation")
                    return unit
...
def create_property_and_unit(property_data, unit_data, address_data, user=None):
    ...
    try:
        with transaction.atomic():
            ...
            property_instance = create_property_with_address(
                property_data, address_data, user
            )
            print(f"DEBUG: Property created with ID: {property_instance.id}")

            # Then create unit linked to the property
            if unit_instance := Unit.objects.filter(
                property=property_instance,
                ...
                unit_instance = create_unit_for_property(
                    property_instance, unit_data, user
                )

            print(
                f"DEBUG: Unit created with ID: {unit_instance.id}, linked to property: {unit_instance.property_id}"
            )
...
def process_address_data(address_data):
    ...
    if "suburb_id" in processed_data:
        try:
            ...
            raise ValidationError(
                f"Suburb with ID {processed_data['suburb_id']} not found"
            )
    else:
        ...
        if isinstance(processed_data["country"], str):
            try:
                ...
                try:
                    ...
                    raise ValidationError(
                        f"Country '{processed_data['country']}' not found"
                    )
            ...
            try:
                ...
                raise ValidationError(
                    f"Country with ID {processed_data['country']} not found"
                )
    ...
    if "city" in processed_data:
        if isinstance(processed_data["city"], str):
            try:
                ...
                if "country" in processed_data and isinstance(
                    processed_data["country"], Country
                ):
                    city = City.objects.create(
                        name=processed_data["city"], country=processed_data["country"]
                    )
                    processed_data["city"] = city
                else:
                    raise ValidationError(
                        "Cannot create city without country reference"
                    )
            ...
            try:
                ...
                raise ValidationError(
                    f"City with ID {processed_data['city']} not found"
                )
