...
class CompanyViewSet(BaseViewSet):
    ...
    @CacheService.cached(
        tag_prefix="choices:location", timeout=CacheService.LONG_CACHE_TIMEOUT
    )
    def get_location_data(self, request):
        ...
        if not country_id:
            countries = Country.objects.filter(is_active=True).values(
                "id", "name", "code"
            )
        ...
        if city_id:
            suburbs = Suburb.objects.filter(city_id=city_id, is_active=True).values(
                "id", "name"
            )
    ...
    def task_status(self, request, task_id=None):
        ...
        try:
            ...
            return self._create_rendered_response(
                {"status": "error", "error": "Failed to check task status"},
