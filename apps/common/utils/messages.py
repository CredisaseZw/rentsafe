...
def prepare_lease_creation_context(lease, tenant=None, for_email=False):
    ...
    context = {
        ...
        'property': getattr(getattr(getattr(lease, 'unit', None), 'property', None), 'name', 'N/A'),
        'unit': getattr(getattr(lease, 'unit', None), 'unit_number', 'N/A'),
        'currency': getattr(getattr(lease, 'currency', None), 'currency_code', 'USD'),
        'start_date': getattr(lease, 'start_date', timezone.now().date()).strftime("%d-%b-%Y"),
        'end_date': getattr(lease, 'end_date', timezone.now().date()).strftime("%d-%b-%Y"),
...
def prepare_lease_payment_context(lease, amount, payment_date=None, tenant=None, for_email=False):
    """
...
def prepare_risk_status_context(lease, old_risk_status, last_payment_date=None, risk_improved=False, tenant=None):
    ...
    return {
        ...
        'last_payment_date': last_payment_date.strftime("%d-%b-%Y") if last_payment_date else 'Never',
...
def prepare_lease_renewal_context(lease, days_until_expiry, tenant=None):
    ...
    notice_period_days = getattr(lease, 'notice_period_days', 
                                getattr(settings, 'DEFAULT_NOTICE_PERIOD_DAYS', 30))
