...
#     Args:
#         response: HttpResponse or Response object
#         access_token (str): JWT access token
#         refresh_token (str, optional): JWT refresh token
    
#     Returns:
#         HttpResponse/Response: Response with cookies set
#     """
#     try:
#         # Convert Response to HttpResponse if needed for cookie operations
#         if isinstance(response, Response):
#             http_response = HttpResponse(
#                 response.rendered_content if hasattr(response, 'rendered_content') else response.data,
...
#             key=settings.SIMPLE_JWT.get('AUTH_COOKIE', 'access_token'),
#             value=access_token,
#             max_age=settings.SIMPLE_JWT.get('ACCESS_TOKEN_LIFETIME', timedelta(minutes=60)).total_seconds(),
#             expires=datetime.now() + settings.SIMPLE_JWT.get('ACCESS_TOKEN_LIFETIME', timedelta(minutes=60)),
...
#                 expires=datetime.now() + settings.SIMPLE_JWT.get('REFRESH_TOKEN_LIFETIME', timedelta(days=7)),
#                 secure=settings.SIMPLE_JWT.get('AUTH_COOKIE_SECURE', True),
#                 httponly=settings.SIMPLE_JWT.get('AUTH_COOKIE_HTTP_ONLY', True),
#                 samesite=settings.SIMPLE_JWT.get('AUTH_COOKIE_SAMESITE', 'Lax'),
#                 path=settings.SIMPLE_JWT.get('AUTH_COOKIE_PATH', '/'),
...
#         # If original was a Response, copy cookies back
#         if isinstance(response, Response):
#             for key, value in http_response.cookies.items():
#                 response.cookies[key] = value
        
#         return response if isinstance(response, Response) else http_response
...
#     Returns:
#         HttpResponse/Response: Response with cookies deleted
#     """
#     try:
#         # Convert Response to HttpResponse if needed
#         if isinstance(response, Response):
#             http_response = HttpResponse(
#                 response.rendered_content if hasattr(response, 'rendered_content') else response.data,
#                 status=response.status_code,
...
# def create_response_with_cookies(data, status_code, access_token=None, refresh_token=None):
    """
    Create a DRF Response with JWT cookies set.
    ...
    Returns:
        Response: DRF Response with cookies
    """
