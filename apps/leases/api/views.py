...
class LeaseViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        try:
            ...
            return Response(
                LeaseDetailSerializer(lease).data, status=status.HTTP_201_CREATED
            )
    ...
    def terminate(self, request, lease_id=None):
        ...
        try:
            ...
            termination, created = LeaseTermination.objects.get_or_create(
                lease=lease, **serializer.validated_data
            )
            ...
            lease.unit.save()
            if notify_tenant:
                self._send_lease_status_notification(
    ...
    def add_tenant(self, request, lease_id=None):
        lease = self.get_object()
        serializer = LeaseTenantSerializer(
        ...
        try:
            ...
            return Response(
                {"error": f"Failed to add tenant: {str(e)}"},
    ...
    def remove_tenant(self, request, lease_id=None):
        lease = self.get_object()
        association_id = request.data.get(
            "association_id"
        )  # Now we need association_id
        ...
        association = get_object_or_404(
            LeaseTenantAssociation, id=association_id, lease=lease
        )

        # Prevent removing the only primary tenant
        if (
            association.is_primary_tenant
            and lease.leasetenantassociation_set.filter(is_primary_tenant=True).count()
            == 1
        ):
            return Response(
                {
    ...
    def set_primary_tenant(self, request, lease_id=None):
        ...
        lease.leasetenantassociation_set.filter(is_primary_tenant=True).update(
    ...
    def set_landlord(self, request, lease_id=None):
        ...
        try:
            ...
            if lease.landlord and lease.landlord.id == landlord_instance.id:
                return Response(
                    {"error": "This landlord is already assigned to this lease."},
                    status=status.HTTP_400_BAD_REQUEST,
                )
            ...
            return Response(
                LeaseDetailSerializer(lease).data, status=status.HTTP_200_OK
            )
        except Exception as e:
            return Response(
                {"error": f"Failed to set landlord: {str(e)}"},
                status=status.HTTP_400_BAD_REQUEST,
            )
    ...
    def remove_landlord(self, request, lease_id=None):
        ...
        if not lease.landlord:
            return Response(
                {"status": "No landlord to remove."}, status=status.HTTP_200_OK
            )
        lease.landlord = None
        lease.save()

        return Response(
            {"status": "Landlord removed from lease."}, status=status.HTTP_200_OK
        )
    ...
    def set_guarantor(self, request, lease_id=None):
        ...
        try:
            ...
            if lease.guarantor and lease.guarantor.id == guarantor_instance.id:
                return Response(
                    {"error": "This guarantor is already assigned to this lease."},
                    status=status.HTTP_400_BAD_REQUEST,
                )
            ...
            return Response(
                {"error": f"Failed to set guarantor: {str(e)}"},
                status=status.HTTP_400_BAD_REQUEST,
            )
    ...
    def remove_guarantor(self, request, lease_id=None):
        ...
        if not lease.guarantor:
            return Response(
                {"status": "No guarantor to remove."}, status=status.HTTP_200_OK
            )
    ...
    def search(self, request):
        ...
        queryset = Lease.objects.filter(
            managing_client=request.user.client
        ).prefetch_related("lease_tenants__tenant_object", "unit__property")
    ...
    def make_payment(self, request, lease_id=None):
        ...
        try:
            with transaction.atomic():
                ...
                primary_tenant_association = lease.leasetenantassociation_set.filter(
                    is_primary_tenant=True
                ).first()
                primary_tenant = (
                    primary_tenant_association.tenant
                    if primary_tenant_association
                    else None
                )
                ...
                self._send_payment_notification(
                    lease, amount, payment_date, primary_tenant
                )
    ...
    def bulk_make_payment(self, request):
        ...
        for payment_data in request.data.get("payments", []):
            ...
            cashbook_id = payment_data.get("cashbook_id")
            try:
                with transaction.atomic():
                    ...
                    if not lease:
                        ...
                        continue
                    primary_tenant_association = (
                        lease.leasetenantassociation_set.filter(
                            is_primary_tenant=True
                        ).first()
                    )
                    primary_tenant = (
                    ...
                    method = PaymentMethod.objects.get(id=method_id)
                    cashbook = (
                        CashBook.objects.get(id=cashbook_id) if cashbook_id else None
                    )
    ...
    def payment_history(self, request, lease_id=None):
        lease = self.get_object()
        payments = (
            Payment.objects.filter(invoice__lease=lease)
            .select_related("invoice", "method")
        ...
        opening_balance_date = (
            lease.opening_balance.date_created
            if hasattr(lease, "opening_balance")
            else None
        )
        if page is not None:
            serializer = PaymentSerializer(
                page, many=True, context={"remaining_balances": remaining_balances}
            )
        ...
        serializer = PaymentSerializer(
            payments_list, many=True, context={"remaining_balances": remaining_balances}
        )
        response_data = {
    ...
    def tenant_statement(self, request):
        ...
        serializer = TenantStatementsListSerializer(
    ...
    def landlord_statement(self, request, lease_id=None):
        ...
        # Calculate totals
        total_disbursements = disbursements.aggregate(total=Sum("amount"))[
            "total"
        ] or Decimal("0.00")
        ...
        return Response(
            {
                "landlord": {
                    ...
                    "commission_percentage": getattr(
                        lease.landlord, "commission_percentage", 0
                    ),
                },
            ...
            result.append(
                {
                    ...
                    "commission_percentage": getattr(
                        landlord, "commission_percentage", 0
                    ),
                    "current_balance": str(current_balance),
    ...
    def _send_lease_creation_notification(self, lease, request):
        ...
        for association in lease.leasetenantassociation_set.all():
            ...
            recipient_type, recipient_id, recipient_name = determine_recipient_info(
                tenant
            )
    ...
    def _send_payment_notification(
        self, lease, amount, payment_date, primary_tenant=None
    ):
        """Send payment confirmation notification"""
        from apps.common.utils.messages import prepare_lease_payment_context
        from apps.leases.utils.notification_utils import (
            determine_recipient_info,
            get_primary_tenant,
            get_any_tenant,
        )
    ...
    def _send_lease_status_notification(self, lease, new_status, reason=None):
        ...
        for tenant_relation in lease.lease_tenants.all():
            ...
            # Prepare context
            context = prepare_lease_status_context(
                lease, new_status, status_display, reason, tenant
            )
            context.update(
                {
                    "platform_name": getattr(settings, "PLATFORM_NAME", "Fincheck"),
    ...
    def _send_risk_status_notification(self, lease, old_risk_status):
        ...
        risk_improved = risk_levels.index(lease.risk_level) < risk_levels.index(
            old_risk_status
        )
        ...
        for tenant_relation in lease.lease_tenants.all():
            ...
            # Prepare context
            context = prepare_risk_status_context(
                lease, old_risk_status, last_payment_date, risk_improved, tenant
            )
            context.update(
                {
                    "platform_name": getattr(settings, "PLATFORM_NAME", "Fincheck"),
