...
class PaymentSerializer(serializers.ModelSerializer):
    ...
    def to_representation(self, instance):
        ...
        if instance.method and instance.method.payment_method_name.lower().startswith(
            "cash"
        ):
        ...
        return {
            "id": instance.id,
            "invoice_number": (
                instance.invoice.document_number if instance.invoice else None
            ),
    ...
    def get_method(self, obj):
        return obj.invoice.currency.symbol if obj.invoice else "$"
    ...
    def validate(self, data):
        ...
        if tenant_type not in ["individual", "company"]:
            raise serializers.ValidationError(
                "Invalid tenant type. Must be 'individual' or 'company'."
            )
        ...
        try:
            ...
            raise serializers.ValidationError(
                f"Invalid content type for {tenant_type}."
            )
...
class GuarantorSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if guarantor_type not in ["individual", "company_branch"]:
            raise serializers.ValidationError(
                "Invalid guarantor type. Must be 'individual' or 'company_branch'."
            )

        try:
            content_type = ContentType.objects.get(
                app_label=(
                    "individuals" if guarantor_type == "individual" else "companies"
                ),
            ...
            raise serializers.ValidationError(
                f"Invalid content type for {guarantor_type}."
            )
        except Exception as e:
...
class LeaseDetailSerializer(serializers.ModelSerializer):
    tenants = LeaseTenantAssociationSerializer(
        many=True, source="leasetenantassociation_set", read_only=True
    )
    ...
    def get_landlord_opening_balances_data(self, obj):
        if hasattr(obj.landlord, "opening_balances"):
            ...
            landlord_balances = LandlordOpeningBalance.objects.filter(
        ...
        return {
            ...
            "property": {
                ...
                "addresses": AddressSerializer(
                    obj.unit.property.addresses.all(), many=True
                ).data,
            },
        }
    ...
    def get_unit(self, obj):
        return {
            ...
            "property": {
                ...
                "name": obj.unit.property.name,
                "type": (
                    obj.unit.property.property_type.name
                    if obj.unit.property.property_type
                    else None
                ),
                "slug": obj.unit.property.slug,
                "addresses": AddressSerializer(
...
class LeaseSearchSerializer(serializers.ModelSerializer):
    ...
    def get_tenants(self, obj):
        return [
            {
                "name": (
                    lt.tenant_object.full_name
                    if hasattr(lt.tenant_object, "full_name")
                    else str(lt.tenant_object)
                ),
    ...
    def create(self, validated_data):
        ...
        with transaction.atomic():
            property = Property.objects.create(
                property_type=property_type, **validated_data
            )
...
class UnitCreateSerializer(serializers.ModelSerializer):
    class Meta:
        model = Unit
        fields = ["unit_number", "unit_type", "number_of_rooms", "status", "features"]
...
class LeaseCreateUpdateSerializer(serializers.ModelSerializer):
    ...
    lease_opening_balance_data = LeaseOpeningBalanceSerializer(
        required=False, allow_null=True
    )
    landlord_opening_balances_data = LandlordOpeningBalanceSerializer(
    ...
    def validate(self, data):
        ...
        property_name = data.get("property_data", {}).get("name") or "Full Property"
        if not data.get("property_data"):
            raise serializers.ValidationError(
                "Please provide property data or create a new property."
            )
        if not data.get("unit_data"):
            data["unit_data"] = {}

        if not user.client:
            raise serializers.ValidationError(
                "User should be associated with a client, request to join one to the admins."
            )
        if not check_rentsafe_subscription(user.client, "rentsafe"):
            raise serializers.ValidationError(
                "No active Rentsafe subscription found or the subscription has expired."
            )

        if (
            data.get("property_data")
            and Lease.objects.filter(
                unit__property__addresses__street_address=data["address_data"].get(
                    "street_address"
                ),
                unit__unit_number=data["unit_data"].get("unit_number", property_name),
                status="ACTIVE",
            ).exists()
        ):
            raise serializers.ValidationError(
