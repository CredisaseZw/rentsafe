...
def create_lease_with_dependencies(lease_data, user=None):
    ...
    try:
        with transaction.atomic():
            ...
            lease_opening_balance_data = data_for_lease_creation.pop(
                "lease_opening_balance_data", None
            )
            landlord_opening_balances_data = data_for_lease_creation.pop(
            ...
            if not unit_instance and property_data:
            ...
            LeaseOpeningBalance.objects.create(
                lease=lease, **lease_opening_balance_data, created_by=user
            )
        ...
        logger.error(
            f"Error in create_lease_with_dependencies: {str(e)}", exc_info=True
        )
...
def create_or_get_landlord(landlord_data):
    ...
    try:
        ...
        raise ValidationError(
            f"{landlord_type.capitalize()} with ID {landlord_id} not found"
        )
    except Exception as e:
...
def create_or_get_guarantor(guarantor_data):
    ...
    if not guarantor_type or not guarantor_id:
        raise ValidationError(
            "Guarantor data must include guarantor_type and guarantor_id"
        )
    ...
    try:
        ...
        guarantor = Guarantor.objects.filter(
            content_type=content_type, object_id=guarantor_id
        ).first()
...
def create_lease_tenant(lease, tenant_data, user=None):
    ...
    try:
        ...
        lease_tenant, created = LeaseTenant.objects.get_or_create(
            content_type=content_type, object_id=tenant_id
        )
